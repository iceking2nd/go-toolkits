...
-----END PUBLIC KEY-----`)
	...
	var data = "iceking2nd test"

	p, _ := RsaEncrypt([]byte(data), publickey)

	if e, err := RsaDecrypt(p, privatekey); err != nil {
		t.Error(err)
	} else {
		if string(e) != data {
			t.Errorf("RsaDecrypt(%v,%s) = %s, expected %s", p, privatekey, e, data)
		}
	}
}
